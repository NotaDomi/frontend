  ...
  const [message,setMessage] = useState('')
    ...
    useEffect( () => {
      ...
      const sendMessage = (event) => { 
        ...
        axios.post('https://www.api.notagram.live/api/messages/send', {
            sender: loggedUser.id,
            receiver: friend.id,
            content: message
        }).then( () => {
            axios.get(`https://www.api.notagram.live/api/messages/getMessages/${loggedUser.id}/${friend.id}`).then( res => {
